...
'///////////////////////-----VR Room-----///////////////////////
Const VRRoom = 3 ' Quest VR override (was 0) ' 0 - VR Room off, 1 - 360 Room, 2 - Minimal Room, 3 - Ultra Minimal
...
SolModCallback(27) = "SetLamp 127,"		'Electric Chair x2
...
sub fcoffin_Timer()
	...
	coffinbottom.blenddisablelighting = 14 * flashx3
	...
	For b = 0 to UBound(BOT)
		If BallVel(BOT(b)) > 1 Then
			...
			Else 'Ball on Raised Ramp
				PlaySound ("BallRoll_" & b), -1, VolPlayfieldRoll(BOT(b)) * 100 * VolumeDial, AudioPan(BOT(b)), 0, PitchPlayfieldRoll(BOT(b))+50000, 1, 0, AudioFade(BOT(b))
			End If
    ...
    For b = 0 to UBound(BOT)
		...
		If BOT(b).Z > 28 then
			if BOT(b).UserValue = 1 Then
				...
				BallShadow(b).RotX = 0
				BallShadow(b).size_x = 6.5 * ((BOT(b).Z-(ballsize/2))/70)
				BallShadow(b).size_y = 5.0 * ((BOT(b).Z-(ballsize/2))/70)
			Else
			...
			BallShadow(b).Z = (b+1) / 100 '0.01 - 0.09 to avoid z-fighting
			BallShadow(b).RotX = 0
			BallShadow(b).size_x = 6.5
			BallShadow(b).size_y = 5.0
			BOT(b).UserValue = 0
		end If
    Next
End Sub
